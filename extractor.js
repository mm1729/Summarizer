...
function getText(size) {
    ...
    console.log(title);
    var xpathQuery = '//body//*[not(self::script or self::style or self::i or \
        self::b or self::strong or self::span or self::a or self::noscript)]/text()\
    ...
    for ( var i=0 ; i < nodesSnapshot.snapshotLength; i++ )
        lenArr.push(nodesSnapshot.snapshotItem(i).textContent.length);

    // find the title of the page -- that is where the article usually starts
    var titleNode = '';
    for ( var i=0 ; i < nodesSnapshot.snapshotLength; i++ ) {
        var textNode = nodesSnapshot.snapshotItem(i).textContent;
        if(title.toLowerCase().indexOf(textNode.toLowerCase(), 0) != -1) {
            titleNode = textNode;
            continue;
        }
        textNode = textNode.replace(/(<([^>]+)>)/ig, "");
        if(textNode.length > 20) all_text.push(textNode);
    }
    return {titleNode, all_text};
}
